...
function TipoTrabajo() {
  ...
  const filteredTipoTrabajo = tipoTrabajo.filter((item) =>
    item[filter].toString().toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleDelete = async (id) => {
    try {
      toast.success("Tipo de trabajo eliminado");
    } catch (error) {
      console.error(error);
      ...
      <main className="pl-52 min-h-screen bg-gray-100 dark:bg-gray-900 dark:text-gray-200">
        <section className="pt-5">
          <article className="flex justify-between items-center">
            <h2 className="text-4xl font-semibold">Tipo de trabajo</h2>
                  ...
                  >
                    ...
                    <th className="px-6 py-4 space-x-3">
                      ...
                      <button
                        onClick={() => {
                          toast.info(
                            'Desea eliminar el tipo de trabajo "' +
                              item.nombre +
                              '"?',
                            {
                              action: {
