...
function FormaPagoView() {
  ...
  const filteredFormaPago = formaPago.filter((item) =>
    item[filter].toString().toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleDelete = async (id) => {
    try {
      toast.success("Forma de pago eliminado");
    } catch (error) {
      console.error(error);
      ...
      <main className="pl-52 min-h-screen bg-gray-100 dark:bg-gray-900 dark:text-gray-200">
        <section className="pt-5">
          <article className="flex justify-between items-center">
            <h2 className="text-4xl font-semibold">Forma de pago</h2>
